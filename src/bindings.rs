...
// TODO replace with tokenregistry structures
...
/// Helper struct for reading binding data.
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct Bindings {
...
impl Bindings {
    ...
    /// * `address` - The address of the Bindings contract.
    ...
    /// Gets an entry from the Bindings by its string-formatted name.
    ///
    /// # Parameters
    /// - `path`: The name-path to get from the Bindings.
    /// # Returns
    ...
    /// Gets an entry from the Bindings by its hash.
    ///
    /// # Parameters
    /// - `entryhash`: The entry to get from the Bindings.
    /// # Returns
    /// A `Result<(Address, Address, Option<Bytes>), EthError>` representing the TBA, owner,
    /// and value if the entry exists and is a note.
    ...
    pub fn get_bootstrap_log_cache(
        ...
        print_to_terminal(2,
                          &format!("get_bootstrap_log_cache (using local binding-cacher): from_block={:?}, retry_params={:?}, chain={:?}",
